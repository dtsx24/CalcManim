...
class Conjuntos_cerrados(Scene):
    def construct(self):
        ...
        cerrado = SVGMobject(
            "Topologia_SVGs/cerrado.svg", color=WHITE, fill_color=BLUE_B
        )
...
class ConjuntosConexos(Scene):
    def construct(self):
        ...
        el_disconexo = TextMobject(
            " A", " es un conjunto", " disconexo", " sí y sólo si "
        ).to_edge(UP)
        ...
        # Conjuntos abiertos
        abiertos = (
            TextMobject("Existen ", "U", " y", " V", " abiertos tales que:")
            .scale(0.9)
            .shift(2 * UP + 3 * RIGHT)
        )
        ...
        conex_b = TextMobject("si cualesquiera abiertos U y V no cumplen").next_to(
            conex_a, DOWN
        )
        ...
        cfinal_a = TextMobject(
            "¡Si un conjunto es", " conexo", ", ningún par de abiertos"
        )
        cfinal_a[1].set_color(BLUE)
        cfinal_b = TextMobject("podrá satisfacer las tres propiedades!").next_to(
            cfinal_a, DOWN
        )
